...
Netflix.prototype.switchProfile = util.promisify(
  Netflix.prototype.switchProfile
);
Netflix.prototype.getRatingHistory = util.promisify(
  Netflix.prototype.getRatingHistory
);
Netflix.prototype.getViewingHistory = util.promisify(
  Netflix.prototype.getViewingHistory
);
Netflix.prototype.setStarRating = util.promisify(
  Netflix.prototype.setStarRating
);
Netflix.prototype.setThumbRating = util.promisify(
  Netflix.prototype.setThumbRating
);
...
main.waterfall = async function (promises) {
  return promises.reduce(
    (promiseChain, currPromise) => promiseChain.then(currPromise),
    Promise.resolve()
  );
};
...
main.getProfileGuid = async function (netflix, profileName) {
  ...
  try {
    ...
    console.error(e);
    throw new Error(
      "Profile GUID could not be determined. For more information, please see previous log" +
        "statements."
    );
  }

  const profileWithCorrectName = profiles.find(
    (profile) => profile.firstName === profileName
  );
...
main.switchProfile = async function (netflix, guid) {
  try {
    ...
    console.error(e);
    throw new Error(
      "Could not switch profiles. For more information, please see previous log statements."
    );
  }
};
...
main.getRatingHistory = async function (netflix) {
  ...
  try {
    ...
    console.error(e);
    throw new Error(
      "Could not retrieve rating history. For more information, please see previous log statements."
    );
  }
};
...
main.getViewingHistory = async function (netflix) {
  ...
  try {
    ...
    console.error(e);
    throw new Error(
      "Could not retrieve viewing history. For more information, please see previous log satements."
    );
  }
};
...
main.readDataFromChosenInput = (fileName) => {
  ...
  if (Array.isArray(data)) {
    ...
    if (Array.isArray(data.ratingHistory)) {
      ...
      throw new Error(
        "Expected data.ratingHistory to be an Array, instead found " +
          JSON.stringify(data.ratingHistory)
      );
    }
    ...
    if (Array.isArray(data.viewingHistory)) {
      ...
      throw new Error(
        "Expected data.viewingHistory to be an Array, instead found " +
          JSON.stringify(data.viewingHistory)
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Expected data.version to be a string like 1.2.3, instead found " +
          data.version
      );
    }
    ...
    throw new Error(
      "An unexpected Error occurred while reading the data to be imported."
    );
  }
